...
float
InstantAssimilation(SimUnit *Grid,
                    float    KDiffuse,
                    float    EFF,
                    float    AssimMax,
                    float    SinB,
                    ...
                    float    PARDirect)
{
    ...
    int          i;
    float        AbsorbedRadiationDiffuse, AbsorbedRadiationTotal,
                 AbsorbedRadiationDirect;
    ...
    Reflection = (1. - sqrt(1. - ScatCoef)) / (1. + sqrt(1. - ScatCoef)) *
                 (2 / (1 + 1.6 * SinB));
    ...
    for (i = 0; i < 3; i++) {
        ...
        /* Absorbed radiation */
        AbsorbedRadiationDiffuse = (1. - Reflection) * PARDiffuse * KDiffuse *
                                   exp(-KDiffuse * LAIC);
        AbsorbedRadiationTotal = (1. - Reflection) * PARDirect * KDirectTl *
                                 exp(-KDirectTl * LAIC);
        AbsorbedRadiationDirect = (1. - ScatCoef) * PARDirect * KDirectBl * exp(
            -KDirectBl * LAIC);

        /* Absorbed flux in W/m2 for shaded leaves and assimilation */
        AbsorbedShadedLeaves = AbsorbedRadiationDiffuse +
                               AbsorbedRadiationTotal - AbsorbedRadiationDirect;
        tmp_max = max(2.0, AssimMax);
        AssimShadedLeaves = AssimMax *
                            (1. - exp(-AbsorbedShadedLeaves * EFF / tmp_max));
        ...
        if (AbsorbedDirectLeaves <= 0) {
            ...
            tmp_max = max(2.0, AssimMax);
            AssimSunlitLeaves = AssimMax *
                                (1. - (AssimMax - AssimShadedLeaves) *
                                 (1 -
                                  exp(-AbsorbedDirectLeaves * EFF / tmp_max)) /
                                 (EFF * AbsorbedDirectLeaves));
        }

        /*  Fraction of sunlit leaf area and local assimilation rate  */
        FractionSunlitLeaves = exp(-KDirectBl * LAIC);
        AssimTotal = FractionSunlitLeaves * AssimSunlitLeaves +
                     (1. - FractionSunlitLeaves) * AssimShadedLeaves;
...
/*-----------------------------------------------------------------------------*/
float
DailyTotalAssimilation(SimUnit *Grid)
{
    extern float XGauss[];
    extern float WGauss[];

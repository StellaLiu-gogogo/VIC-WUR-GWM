...
crop_alloc(void)
{
    ...
    for (i = 0; i < local_domain.ncells_active; i++) {
        ...
        crop_con_map[i].cidx =
            malloc(crop_con_map[i].nc_types * sizeof(*crop_con_map[i].cidx));
        check_alloc_status(crop_con_map[i].cidx, "Memory allocation error");

        crop_con_map[i].veg_class =
            malloc(crop_con_map[i].nc_types *
                   sizeof(*crop_con_map[i].veg_class));
        check_alloc_status(crop_con_map[i].veg_class,
                           "Memory allocation error");

        crop_con_map[i].Cc =
            malloc(crop_con_map[i].nc_types * sizeof(*crop_con_map[i].Cc));
        check_alloc_status(crop_con_map[i].Cc, "Memory allocation error");

        for (j = 0; j < crop_con_map[i].nc_types; j++) {
            crop_con_map[i].Cc[j] =
                malloc(MONTHS_PER_YEAR * sizeof(*crop_con_map[i].Cc[j]));
            check_alloc_status(crop_con_map[i].Cc[j],
                               "Memory allocation error");
        }
        ...
        crop_force[i].N_amount =
            malloc(crop_con_map[i].nc_active * sizeof(*crop_force[i].N_amount));
        check_alloc_status(crop_force[i].N_amount, "Memory allocation error");

        crop_force[i].P_amount =
            malloc(crop_con_map[i].nc_active * sizeof(*crop_force[i].P_amount));
        check_alloc_status(crop_force[i].P_amount, "Memory allocation error");

        crop_force[i].K_amount =
            malloc(crop_con_map[i].nc_active * sizeof(*crop_force[i].K_amount));
        check_alloc_status(crop_force[i].K_amount, "Memory allocation error");

        for (j = 0; j < crop_con_map[i].nc_active; j++) {
            crop_con[i][j].DVS_point = malloc(
                plugin_options.NFERTTIMES * sizeof(*crop_con[i][j].DVS_point));
            check_alloc_status(crop_con[i][j].DVS_point,
                               "Memory allocation error");

            crop_con[i][j].N_amount =
                malloc(plugin_options.NFERTTIMES *
                       sizeof(*crop_con[i][j].N_amount));
            check_alloc_status(crop_con[i][j].N_amount,
                               "Memory allocation error");

            crop_con[i][j].P_amount =
                malloc(plugin_options.NFERTTIMES *
                       sizeof(*crop_con[i][j].P_amount));
            check_alloc_status(crop_con[i][j].P_amount,
                               "Memory allocation error");

            crop_con[i][j].K_amount =
                malloc(plugin_options.NFERTTIMES *
                       sizeof(*crop_con[i][j].K_amount));
            check_alloc_status(crop_con[i][j].K_amount,
                               "Memory allocation error");

            crop_force[i].DVS_point[j] = malloc(
                plugin_options.NFERTTIMES *
                sizeof(*crop_force[i].DVS_point[j]));
            check_alloc_status(crop_force[i].DVS_point[j],
                               "Memory allocation error");

            crop_force[i].N_amount[j] = malloc(
                plugin_options.NFERTTIMES * sizeof(*crop_force[i].N_amount[j]));
            check_alloc_status(crop_force[i].N_amount[j],
                               "Memory allocation error");

            crop_force[i].P_amount[j] = malloc(
                plugin_options.NFERTTIMES * sizeof(*crop_force[i].P_amount[j]));
            check_alloc_status(crop_force[i].P_amount[j],
                               "Memory allocation error");

            crop_force[i].K_amount[j] = malloc(
                plugin_options.NFERTTIMES * sizeof(*crop_force[i].K_amount[j]));
            check_alloc_status(crop_force[i].K_amount[j],
                               "Memory allocation error");
        }
    }

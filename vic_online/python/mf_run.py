...
path = os.chdir('/home/sliu/Documents/gitversion/VIC-WUR-GWM-1910/vic_online/python/mfoutput') # folder to save results, don't delete
workspace = '/home/sliu/Documents/gitversion/VIC-WUR-GWM-1910/vic_online/python/mfinput/' # result folder, remember '/' ！！！！

exedir = '/home/sliu/Documents/gitversion/VIC-WUR-GWM-1910/vic_online/python/mfexe/vic_image_gwm_offline.exe/bin/mf6/' # mf6
inputdir = '/home/sliu/Documents/gitversion/VIC-WUR-GWM-1910/vic_online/python/mfinput/' # inputs remember / ！！！
outputdir = '/home/sliu/Documents/gitversion/VIC-WUR-GWM-1910/vic_online/python/mfoutput/' # outputs，remember / ！！！

# origin : Upper Left
clonemap = gdal.Open(inputdir + 'indus_Clone_05min.nc')
#%%
landmask = gdal.Open(inputdir + 'indus_landmask.nc').ReadAsArray().astype(np.float32) # ocean - 0, land - 1 
#%%
dem_ini = gdal.Open(inputdir + 'indus_demfrom30s.nc').ReadAsArray()

aqdepth_ini = gdal.Open(inputdir + 'indus_damc_ave.nc').ReadAsArray() # to define aquifer depth

ksat_log = gdal.Open(inputdir + 'indus_lkmc_ave.nc').ReadAsArray() # to define K

cellarea = gdal.Open(inputdir + 'indus_Indus_CellArea_m2_05min.nc').ReadAsArray() # cell of Indus

qbank = gdal.Open(inputdir+'indus_Qbankfull_edwinInputs.nc').ReadAsArray() 

riv_slope1 =gdal.Open(inputdir+'indus_slope05min_avgFrom30sec.nc').ReadAsArray()

Z0_floodplain = gdal.Open(inputdir+'indus_efplact_new_05min.nc').ReadAsArray()

qaverage = gdal.Open(inputdir + 'indus_mean_discharge_edwinInput.nc').ReadAsArray() # q of river

min_dem=gdal.Open(inputdir+'indus_mindem_05min.nc').ReadAsArray()

KQ3 = gdal.Open(inputdir + 'indus_Recess_NEW.nc').ReadAsArray()

conflayers = gdal.Open('/lustre/nobackup/WUR/ESG/yuan018/03DataVisual/conflayers4.nc').ReadAsArray()
conflayers = conflayers[1355:1535, 2951:3155]
...
ts_gwrecharge = gdal.Open(inputdir+'gwRecharge_month_1968to2000.nc').ReadAsArray()
ts_gwrecharge = np.flip(ts_gwrecharge, axis = 1)
ts_discharge = gdal.Open(inputdir+ 'riverDischarge_month_1968to2000.nc').ReadAsArray()
ts_discharge = np.flip(ts_discharge, axis = 1)

nc_boundary = nc.Dataset(inputdir + 'boundary.nc') # bdmask origin: bot-left
bdmask = nc_boundary.variables['bdmask'][:].data # 0 for inactive cells; 1 for boundary cells; 2 for active cells
# bdmask = nc_boundary.variables['boundary'][:].data # 0 for inactive cells; 1 for boundary cells; 2 for active cells
bdmask = np.flip(bdmask , axis = 0)

topl1_gwl = nc.Dataset(inputdir + 'topl1_gwl_Indus_monthly_1968to2000.nc')
gwll1= topl1_gwl.variables['gwl'][:].data
gwll1 = np.flip(gwll1, axis = 1)
...
ibound = np.zeros_like(landmask)
ibound = np.copy(landmask)
ibound[landmask == 0] = -1 # differentiate land and ocean
ibound = ibound.astype(int) # ibound: ocean - -1, land - 1


idomain = np.zeros_like(bdmask) # differentiate grids in calculation and not in calculation
idomain = np.copy(bdmask)
idomain = idomain.astype(int) # indus boundary - 1，inside - 2，outside - 0

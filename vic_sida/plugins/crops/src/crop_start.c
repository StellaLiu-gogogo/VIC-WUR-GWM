...
crop_start(void)
{
    ...
    if (plugin_options.WOFOST_FORCE_FERT) {
        for (i = 0; i < 4; i++) {
            // Get information from the forcing file(s)
            // Open first-year forcing files and get info
            snprintf(plugin_filenames.forcing[fert_idx[i]].nc_filename,
                     MAXSTRING, "%s%4d.nc",
                     plugin_filenames.f_path_pfx[fert_idx[i]],
                     global_param.startyear);
            status = nc_open(plugin_filenames.forcing[fert_idx[i]].nc_filename,
                             NC_NOWRITE,
            ...
            if (dim_len != plugin_options.NCROPTYPES) {
                log_err(
                    "Fertilizer forcing crop_class length is not equal to NCROPTYPES");
            }
            dim_len = get_nc_dimension(&(plugin_filenames.forcing[fert_idx[i]]),
                                       "fertilizer_times");
            if (dim_len != plugin_options.NFERTTIMES) {
                log_err(
                    "Fertilizer forcing fertilizer_times length is not equal to NFERTTIMES");
            }
